...
2:  Tested on Atmega32A/Atmega1284P @ Arduino IDE 1.8.19 and MightyCore v.2.2.2
    ...
    ***** THIS MULTIFREQUENCY VARIANT ONLY TESTED ON ATMEGA32A, IDE 1.8.13, MIGHTYCORE v.2.1.2 *****
    ***** NOT TESTED WITH Fosc = 20MHZ                                                         *****
...
EDITED BY Sarah K.Libman - sarahkmarr.com - TO ALLOW MULTIPLE Z80 TYPES AND FREQUENCIES
...
#if F_CPU == 20000000
//  #define CLOCK_LOW   "5"               // deleted by SKL
//  #define CLOCK_HIGH  "10"              // deleted by SKL
                      // start added by SKL
  #define CLOCK_16    "1.6"
  #define CLOCK_20    "2.0"
  #define CLOCK_27    "2.7"
  ...
  #define CLOCK_80    "8.0"
                        // end added by SKL
#else
// #define CLOCK_HIGH  "8"                // deleted by SKL
// #define CLOCK_LOW   "4"                // deleted by SKL
										  // start added by SKL
  #define CLOCK_16    "1.6"
  #define CLOCK_20    "2.0"
  #define CLOCK_27    "2.7"
...
const byte    autoexecFlagAddr = 12;      // Internal EEPROM address for AUTOEXEC flag storage
										  // start deleted by SKL
// const byte    clockModeAddr = 13;      // Internal EEPROM address for the Z80 clock high/low speed switch
//                                        // (1 = low speed, 0 = high speed)
										  // end deleted by SKL
										  // start added by SKL
const byte    clockModeAddr = 13;         // Internal EEPROM address
                                          // Holds clockMode +
                                          //             0 if OG Z80 (max 2.6MHz)
                                          //            10 if Z80A (max 4MHz)
                                          //            20 if 'modern' Z80 (max 8MHz)
                                          // end added by SKL							  
...
char          inChar;                     // Input char from serial
										  // start added by SKL
char          inChar2;                    // Input char from serial
char          inChar3;                    // Input char from serial
										  // end added by SKL							  
...
// byte          clockMode;               // Z80 clock HI/LO speed selector (0 = 8/10MHz, 1 = 4/5MHz)
										  // end deleted by SKL
										  // start added by SKL
byte          clockMode;                  // Z80 clock selector (0 = 8MHz, 1 = 4MHz, 2 = 2.7MHz, 3 = 2MHz, 4 = 1.6MHz)
                                          //                (20MHz crystal will give proportional values)
byte          z80Type;                    // type of Z80 : 0 if OG Z80 (max 2.6MHz), 1 if Z80A (max 4MHz), 2 if 'modern' Z80 (max 8MHz)								  
										  // end added by SKL
...
// ------------------------------------------------------------------------------
  ...
  char          maxSelChar    = '8';        // Maximum allowed ASCII value selection (boot selection)
  											// start added by SKL
  char          minZ80TypeChar = '1';       // Minimum allowed ASCII value selection (z80 type selection)
  char          maxZ80TypeChar = '3';       // Maximum allowed ASCII value selection (z80 type selection)
  char          minClockSpeedChar = '1';    // Minimum allowed ASCII value selection (clock speed selection)
  char          maxClockSpeedChar = '3';    // Maximum allowed ASCII value selection (clock speed selection)
  ...
  byte          bootSelection = 0;          // Flag to enter into the boot mode selection
  											// start added by SKL
  byte          minClockMode  = 4;          // Minimum value of clockMode - depends on Z80 type - 4 is lowest speed
  byte          maxClockMode  = 4;          // Maximum value of clockMode - 4 for all Z80 types
...
// ------------------------------------------------------------------------------
  ...
  // clockMode = EEPROM.read(clockModeAddr);      // Read the previous stored value
  												  // end deleted by SKL
  												  // start added by SKL
  // Read the Z80 CPU type and speed mode         // find out what sort of Z80 we're using and set maxClockMode and fix if necessary
  ...
  Serial.println(F("      /  >-< |   | ___ | \\/ ||--: |      .'        IOS rev.   S220718-R290823"));
  ...
  {
    Serial.println();
    Serial.println(F("--------------- WARNING: YOUR Z80-MBC2 IS USING A 20MHz CRYSTAL ---------------"));
    Serial.println(F("- Multiply all reported frequencies by 1.25 (e.g. 4.0MHz = 5.0MHz)            -"));
    Serial.println(F("- The highest available frequency for a given Z80 type will OVERCLOCK by 25%  -"));
    Serial.println(F("-------------------------------------------------------------------------------"));
    Serial.println();
  }
 												                          // end added by SKL
  ...
  Serial.println("IOS    Multifrequency support enabled");
  switch(z80Type)
  {
    case 0:
  ...
  //   or an invalid bootMode code was read from internal EEPROM
  {
    ...
    Serial.println(F("\r\n    4  Autoboot"));
    Serial.println(F("    5  iLoad"));
    Serial.println(F("    6  [multifreq] Change Z80 type and clock speed"));
    Serial.print(F("    7  Toggle CP/M autoexec (->"));
    ...
    // Serial.print(F("Enter your choice >"));
        										  // end deleted by SKL
        										  // start added by SKL
    Serial.print(F("IOS    Enter your choice > ")); 										  
        										  // end added by SKL
    do
    {
      WaitAndBlink(CHECK);
      ...
      case '8':                                   // Change serial port speed
        ...
        // Store new serial speed index
        {
          ...
          // Serial.println(F("Changed speed will be effective after next reboot!\r\n\n"));
												  // end deleted by SKL
												  // start added by SKL
          Serial.println(F("IOS    Changed speed will be effective after next reboot\r\n\n"));
												  // end added by SKL
        }
  ...
  digitalWrite(RESET_, LOW);                      // Activate the RESET_ signal
    ...
    // OCR2 = clockMode;                             // Set the compare value to toggle OC2 (0 = low or 1 = high)
											      // end deleted by SKL
											      // start added by SKL
	OCR2 = clockMode;                             // Set the compare value to toggle OC2  - now 0, 1, 2, 3 or 4
											      // end added by SKL
    ...
    // OCR2A = clockMode;                            // Set the compare value to toggle OC2 (0 = low or 1 = high)
											      // end deleted by SKL
											      // start added by SKL
	OCR2A = clockMode;                            // Set the compare value to toggle OC2  - now 0, 1, 2, 3 or 4
											      // end added by SKL
  #else
  ...
  // Serial.println(F("IOS: Z80 is running from now"));
      											  // end deleted by SKL
      											  // start added by SKL
  Serial.println("IOS    Z80 now running");
      											  // end added by SKL
  Serial.println();
...
// Return value: 0 if RTC not present, 1 if found.
{
  ...
  // Serial.println("C");
  												  // end deleted by SKL
  												  // start added by SKL
  Serial.print("IOS    Found RTC DS3231 (");
  printDateTime(1);
  Serial.print(" ");
  Serial.print((int8_t)tempC);
  Serial.println("C)");
